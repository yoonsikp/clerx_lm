...
RELATION_ONLY = os.getenv('RELATION_ONLY') and os.getenv('RELATION_ONLY') == "1"
...
class InputFeatures:
    ...
    relation_labels: Optional[List[int]] = None
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        relation_labels = [pad_token_label_id]
        for word, label in zip(example.words, example.labels):
            ...
            if len(word_tokens) > 0:
                ...
                if label == 'CONTEXT':
                    # we are assuming roberta here, where segment ids do not matter
        ...
        # Account for [CLS] and [SEP] with "- 2" and with "- 2" for RoBERTa.
